...
func Unmarshal_json(aMap []byte) map[string]interface{} {
	var data map[string]interface{}
	json.Unmarshal(aMap, &data)
	return data
}

//Подписываемся на nats-server, взят из примера на git clone https://github.com/nats-io/stan.go.git
func Create_Subscriber() {
	var (
	...
	// При получаении данных парсим json-файл и отпрвляем его в бд
	subj, i := args[0], 0
	mcb := func(msg *stan.Msg) {
		i++
		//Декодируем json
		data := Unmarshal_json(msg.Data)
		// Парсим и отправляем данные в талицы main, delivery,
		// items, payment соответствующие данные в бд
		id := Parse_json(data)
